...
//        exit($query_post_view_count);
    ...
    public function get_popular_authors_list($start_time, $end_time, $limit = null) {
        ...
        foreach ($authors_data as $author_data) {
            $author_comments_count = $this->db->get_var('SELECT COUNT(`comments`.`comment_ID`) AS `comments_count` FROM `' . $this->db->prefix . 'comments` AS `comments` WHERE `comments`.`comment_approved` = 1 AND `comments`.`comment_author_email` LIKE "' . $author_data['email'] . '"');
    ...
    public function get_author_all_posts_count($author_id, $start_time, $end_time) {
        $select_query = 'SELECT COUNT(posts.`ID`)AS `posts_count` FROM `' . $this->db->prefix . 'posts` AS `posts` WHERE posts.`post_author` = %d AND posts.`post_status` = "publish" AND posts.`post_type` IN(' . $this->post_types . ') AND DATE(`post_date`) >= %s AND DATE(`post_date`) <= %s';
