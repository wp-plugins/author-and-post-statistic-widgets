<div class="separate-wrapper">
    ...
    $author_posts = $this->apsw_db_helper->get_author_all_posts_count($apsw_user_id, $from, $to);
        ...
        <ul class="stats-author-list">
                ...
                <li class="stats-author-categories-list">
                    ...
                    <ul class="nested-list">
                        <?php foreach ($author_cats as $key => $value) { ?>
                            <li>
                                <a href="<?php echo get_category_link($key); ?>" title="<?php echo __('View all posts in category', APSW_Core::$APSW_TEXT_DOMAIN) . ' ' . $value['name']; ?>">
            ...
            <?php if ($author_tags) { ?>
                <li class="stats-author-tags-list">
                    <span class="stats-label"><?php _e('Tags List', APSW_Core::$APSW_TEXT_DOMAIN); ?></span>
                    <span class="stats-value">&nbsp;</span>
                    <ul class="nested-list">                        
                        <?php foreach ($author_tags as $key => $value) { ?>
                            <li>
                                <a href="<?php echo get_tag_link($key); ?>" title="<?php echo __('View all posts tagged as', APSW_Core::$APSW_TEXT_DOMAIN) . ' ' . $value; ?>">
                                    <span class="stats-label">
                                        <?php echo $value . ' '; ?>
                                    </span>
                                </a>
            ...
            <?php if ($author_taxonomies) { ?>
                <li class="stats-author-taxonomies-list">
                    <span class="stats-label"><?php _e('Custom Taxonomies List', APSW_Core::$APSW_TEXT_DOMAIN); ?></span>
                    <span class="stats-value">&nbsp;</span>
                    <ul class="nested-list">
                        <?php foreach ($author_taxonomies as $key => $value) { ?>
                            <li>
                                <span class="stats-label">
                                    <a href="<?php echo get_term_link($key, $value['taxonomy']); ?>" title="<?php echo __('View all posts under', APSW_Core::$APSW_TEXT_DOMAIN) . ' ' . $value['name']; ?>">
    ...
    <?php if (is_array($apsw_user->roles) && !in_array('subscriber', $apsw_user->roles)) { ?>
        <div class="stats-block stats-post-block">
            <span class="inner-title"><?php _e('Posts Statistics', APSW_Core::$APSW_TEXT_DOMAIN); ?></span>         
            <ul class="stats-posts-list">
                <?php
                if ($this->apsw_options_serialized->is_popular_posts_by_post_views == 1) {
                    $author_popular_posts = $this->apsw_db_helper->get_author_popular_posts_by_view_count($apsw_user_id, $from, $to, $posts_limit);
                    ...
                    $author_popular_posts = $this->apsw_db_helper->get_author_popular_posts_by_commment_count($apsw_user_id, $from, $to, $posts_limit);
                    foreach ($author_popular_posts as $author_popular_post) {
                        $post_id = $author_popular_post['post_id'];
                        $post_title = $author_popular_post['p_title'];
                        ...
                        <li class="stats-post-views-count">
                            ...
                            <?php if ($comment_status === "open") { ?>
                                <a href="<?php echo get_comments_link($post_id); ?>" title="<?php echo __('Comment on', APSW_Core::$APSW_TEXT_DOMAIN) . ' ' . $post_title; ?>">
                                    <span class="stats-value">
                                        <?php echo $post_comments_count; ?> <img src="<?php echo plugins_url(APSW_Core::$PLUGIN_DIRECTORY . '/files/img/icon_comments.png') ?>" title="<?php _e('comments', APSW_Core::$APSW_TEXT_DOMAIN) ?>" alt="<?php _e('comments', APSW_Core::$APSW_TEXT_DOMAIN) ?>" align="absmiddle" class="apsw-comments-img" />
                                    </span>
                                </a>
                            <?php } else { ?>
